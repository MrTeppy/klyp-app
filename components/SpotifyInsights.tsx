...
function moodFromRecent(recent: any[], top: Track[]) {
  ...
  const hours = recent
    .map((x) => {
      const playedAt = x?.played_at;
      if (!playedAt) return null;
      return new Date(playedAt).getHours();
    })
  ...
  if (avgHour >= 0 && avgHour < 5) {
    return "After Midnight / inward / replaying the same feeling";
  }
  if (avgHour >= 5 && avgHour < 12) {
    return "Morning Drift / reset / soft momentum";
  }
  if (avgHour >= 12 && avgHour < 18) {
    return "Daylight Blur / moving / detached";
  }
  if (names.includes("radiohead")) {
    return "Grey Glow / overcast / cinematic";
  }
  if (names.includes("the cure")) {
    return "Cold Air / bright sadness / motion";
  }
  if (mostRepeated && mostRepeated[1] >= 2) {
    return `Repeat State / ${mostRepeated[0]} / not over it yet`;
  }
...
export default function SpotifyInsights() {
  ...
  useEffect(() => {
    async function loadSpotify() {
      ...
      try {
        ...
        if (profileRes.status === 401 || topRes.status === 401 || recentRes.status === 401) {
          localStorage.removeItem("spotify_access_token");
          localStorage.removeItem("spotify_refresh_token");
          localStorage.removeItem("spotify_code_verifier");
          setNeedsConnect(true);
          setError("Your Spotify session expired. Connect again.");
          setLoading(false);
          return;
        }
        ...
        setTopTracks(Array.isArray(topData?.items) ? topData.items : []);
        setRecentTracks(Array.isArray(recentData?.items) ? recentData.items : []);

        if (!profileRes.ok && !topRes.ok && !recentRes.ok) {
          setError("Spotify connected, but no data could be read yet.");
        } else if (!topRes.ok || !recentRes.ok) {
          setError("Spotify connected, but some listening data is still unavailable.");
        }
      } catch (err: any) {
        setError("Spotify connected, but data could not load right now.");
      } finally {
        setLoading(false);
      }
    }
  ...
  if (needsConnect) {
    return (
      <div className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm">
        ...
        {error ? <p className="mt-3 text-sm text-black/50">{error}</p> : null}
        <Link
          href="/spotify"
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="mt-4 space-y-3">
          {topTracks.length ? topTracks.map((track, i) => (
            <div key={track.id} className="rounded-2xl bg-[#f8f5ef] p-4">
              ...
              <div className="mt-1 text-[15px] font-medium">{track.name}</div>
      ...
      <div className="rounded-[28px] border border-black/10 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-3">
          {recentTracks.length ? recentTracks.slice(0, 5).map((item, i) => (
            <div key={`${item?.track?.id || "track"}-${i}`} className="rounded-2xl bg-[#f8f5ef] p-4">
              <div className="text-[15px] font-medium">{item?.track?.name || "Unknown track"}</div>
              <div className="text-[13px] text-black/55">
      ...
      {error ? (
        <div className="rounded-[28px] border border-black/10 bg-white p-5 text-sm text-black/55 shadow-sm">
          {error}
        </div>
      ) : null}
    </div>
